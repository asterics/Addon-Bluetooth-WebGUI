    ...
    render() {
        ...
        return html`
            ...
            <h3 class="mt-5">${L.translate('Timings for special functions // Schwellenwerte für Spezialfunktionen')}</h3>
                ...
                <!-- These settings not supported by now in the unified GUI version, TBD. 
                    ...
                    <div class="mb-5 mb-md-2">
                        ${html`
                            <${Slider} id="slider" label="Threshold for multiple button presses [ms], 0=disable: // Schwellenwert für mehrfaches Drücken [ms], 0=deaktivieren:" 
                            oninput="${(value, constants) => this.valueChanged(value, constants)}" 
                            value="${state[C.AT_CMD_THRESHOLD_MULTIPRESS]}" min="0" max="10000" step="100" 
                            updateConstants="${[C.AT_CMD_THRESHOLD_MULTIPRESS]}"/>`}
                    </div>
