...
class TabSipPuff extends Component {
  ...
  render() {
    ...
    <span id="pressureLiveA11yLabel" class="sr-only">${L.translate('Current pressure value // Aktueller WertDruck')}</span>
    <span id="pressureLiveA11y" aria-describedby="pressureLiveA11yLabel" class="onlyscreenreader" role="status"
        aria-live="off" accesskey="w" tabindex="-1">${state.valueA11y}</span>
    ...
    <div id="tab-puff-container" class="relative container-fluid">
        <div class="row back-layer full-height full-width">
            ...
            <div class="ten columns full-height relative">
                ...
                <div id="guide-max" class="back-layer full-height border-right-red"
                    style="width: ${state.percentMax}%;"></div>
                <div id="guide-min" class="back-layer full-height border-right-blue"
                    style="width: ${state.percentMin}%;"></div>
            </div>
        </div>
        ...
        <div id="SIP_THRESHOLD_WRAPPER" class="row ${state.value < state.SIP_THRESHOLD ? 'colored-thumb' : ''}">
            <span aria-hidden="true" id="SIP_THRESHOLD_VAL"
                class="text-center two columns">${state.SIP_THRESHOLD}</span>
            <input type="range" value="${state.SIP_THRESHOLD}"
                oninput="${(event) => this.sliderChanged(event, C.AT_CMD_SIP_THRESHOLD)}" id="SIP_THRESHOLD"
        ...
        <div id="PUFF_THRESHOLD_WRAPPER" class="row ${state.value > state.PUFF_THRESHOLD ? 'colored-thumb' : ''}">
            <span aria-hidden="true" id="PUFF_THRESHOLD_VAL"
                class="text-center two columns">${state.PUFF_THRESHOLD}</span>
            <input type="range" value="${state.PUFF_THRESHOLD}"
                oninput="${(event) => this.sliderChanged(event, C.AT_CMD_PUFF_THRESHOLD)}" id="PUFF_THRESHOLD"
    ...
    <div style="margin-top: 4em">
        ${html`
        <${ActionButton}
            onclick="${() => ATDevice.copyConfigToAllSlots([C.AT_CMD_SIP_THRESHOLD, C.AT_CMD_SIP_STRONG_THRESHOLD, C.AT_CMD_PUFF_THRESHOLD, C.AT_CMD_PUFF_STRONG_THRESHOLD])}"
        ...
        <div class="row" style="margin-top: 1em">
           <div class="col">
           <${ActionButton} resetSlidersPuffSip="${() => this.resetSlidersPuffSip()}" 
           label="Reset All Thresholds // Alle Schwellenwerte zurücksetzen" faIcon="fas undo" progressLabel="Resetting Thresholds ... // Schwellenwerte werden zurückgesetzt..." />
           </div>
        </div>
